 ...
 *
 * @details
 * The compile-time definition of `LOG_LEVEL` controls what prints are included in the compiled
 * program. This definition may either be global (using `-D NDEBUG` when compiling) or set on a
 * per-file basis with a default fallback defined in this file.
 *
 ...
 * - if `NDEBUG` is defined, asserts and non-error prints will do nothing, and their invocations
...
#  define ANSI_COLOR_PUR   "\033[1;35m"
#  define ANSI_COLOR_CYA   "\033[0;36m"
#  define ANSI_COLOR_WHI   "\033[1;37m"
...
#      define COLOR_META
#    else
#      define COLOR_META ANSI_COLOR_WHI
#    endif /* PRINTING_NCOLOR */
...
/**
 * @brief Convenience macro to print error message before aborting (forcefully quitting) the
 * program.
 *
...
#  define PANIC(fmt, ...)                                                                             \
        do {                                                                                          \
            do_print_if_lvl(                                                                          \
                LOG_LEVEL_PANIC,                                                                      \
                stderr,                                                                               \
                COLOR_META META_FILE_LINE_FMT META_FUNC_FMT COLOR_PR_ERROR "PANIC: " COLOR_RESET fmt, \
                META_FILE_LINE_ARGS,                                                                  \
                META_FUNC_ARGS,                                                                       \
                ##__VA_ARGS__                                                                         \
            );                                                                                        \
 ...
 *
 * @note omitted only if `LOG_LEVEL == LOG_LEVEL_PANIC`. Prefix is simplified if `NDEBUG` is
 * defined, as file/line meta info is typically wonky when compiled with optimizations - as
 * `NDEBUG` implies is the case.
 */
#    define pr_error(fmt, ...)                                                                      \
          do_print_if_lvl(                                                                          \
              LOG_LEVEL_ERROR,                                                                      \
              stderr,                                                                               \
              COLOR_META META_FILE_LINE_FMT META_FUNC_FMT COLOR_PR_ERROR "error: " COLOR_RESET fmt, \
              META_FILE_LINE_ARGS,                                                                  \
              META_FUNC_ARGS,                                                                       \
              ##__VA_ARGS__                                                                         \
          )
...
#    define pr_warn(fmt, ...)                                                          \
          do_print_if_lvl(                                                             \
              LOG_LEVEL_WARN,                                                          \
              stderr,                                                                  \
              COLOR_META META_FILE_LINE_FMT COLOR_PR_WARN "warning: " COLOR_RESET fmt, \
              META_FILE_LINE_ARGS,                                                     \
              ##__VA_ARGS__                                                            \
          )
 ...
 *
 * No prefix, safe for multi-line/partial prints. Essentially just a printf wrapper with the added
 * ability to toggle, either globally or per-file.
 *
...
#    define pr_debug(fmt, ...)                                                                      \
          do_print_if_lvl(                                                                          \
              LOG_LEVEL_DEBUG,                                                                      \
              stderr,                                                                               \
              COLOR_META META_FILE_LINE_FMT META_FUNC_FMT COLOR_PR_DEBUG "debug: " COLOR_RESET fmt, \
              META_FILE_LINE_ARGS,                                                                  \
              META_FUNC_ARGS,                                                                       \
              ##__VA_ARGS__                                                                         \
          )
 ...
 *
 * Does nothing unless `assertion` evaluates to false, in which case program execution is aborted
 * and and the literal assertion is printed. (e.g. `assertion '<not-true>' failed\n`)
 *
...
/**
 * @brief Like `assert`, but with a printf-like extension that is executed on assertion failure.
 * Typically used to print the values of the variables.
 *
 * Does nothing unless `assertion` evaluates to false, in which case program execution is aborted
 * and and the literal assertion is printed along with the provided message. (e.g. `assertion
 * '<not-true>' failed: <msg>`). Does not include a newline.
 *
 * @note omitted if `NDEBUG` is defined.
 */
